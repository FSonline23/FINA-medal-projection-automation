...
for (i in 1:length(UniqueSwimmers)) {
  ...
  dataset_used <- "2023"
  remarks <- ""
  results <- filter(swiming_data_2023, full_name_computed == UniqueSwimmers[i])
  top50pctResultsRows <- round(nrow(results) / 2, digits = 0)
  Swimmer <- head(results, top50pctResultsRows)

  # Conditional Checks for Count of Dataframe Object. If less than 5, escalate to next dataset.
  if (nrow(Swimmer) < 3) {
    dataset_used <- "2022-2023"
    remarks <- paste("Not enough data points for 2023 (total of ", nrow(Swimmer), " datapts)")
  ...
  if (nrow(Swimmer) < 3) {
    ...
    remarks <- paste("Not enough data points for 2022-2023 (total of ", nrow(Swimmer), " datapts)")
  ...
  if (nrow(Swimmer) < 3) {
    remarks <- paste("Not enough data points for 2019-2023 (total of ", nrow(Swimmer), " datapts)")
    
    # Append swimmer normality result to dataframe
    df_shapiroTestResults[nrow(df_shapiroTestResults) + 1, ] = c(UniqueSwimmers[i], "NA", dataset_used, nrow(results), top50pctResultsRows, remarks)
    # print(paste("Final Shapiro wilk p-value: NA. Dataset Used:", dataset_used))

    # Append swimmer dataset to dataframe
    df_results = rbind(df_results, Swimmer)
    next
  }
  ...
  if (shap_test$p.value <  0.05) {
    ...
    top50pctResultsRows <- round(nrow(Swimmer) / 2, digits = 0)
  ...
  # Append swimmer normality result to dataframe
  df_shapiroTestResults[nrow(df_shapiroTestResults) + 1, ] = c(UniqueSwimmers[i], shap_test$p.value, dataset_used, nrow(results), top50pctResultsRows, remarks)
